...
def Pr(u, eta):
    if ( u == 0 ):
        ...
        for l in range(1,u+1):
            sum += exp(-eta-u*log(2)+l* log(eta)-lgamma(l+1)+lgamma(u)-lgamma(l)-lgamma(u-l+1))
        p = sum
    return p

def test(sequence, n, blen=6):
    ...
    p = gammainc(5.0/2.0, chisq/2.0) # Compute P value
