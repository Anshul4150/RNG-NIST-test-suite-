...
def main():
    #input file containing bit string 
    data = "jsmp.txt"
    ...
    '''
    this following part creates the csv writer for the results 
    '''
    ...
    for i in range(NUM_TEST):
        if (i<9) :
            output[i] = "results/jsmp/results_0" + str(i+1) + "_" + testlist[i] +".csv"
        else:
            output[i] = "results/jsmp/result_" + str(i+1) +"_" +testlist[i] + ".csv"
        
        if fieldnames[i] != None :
            fo[i] = open(output[i], "w")
    ...
    for i in range(NUM_TEST):
        ...
        writeDict[fieldnames[i][len(fieldnames[i])-2]] = float(success_count)/total_count
        writer[i].writerow(writeDict)
