...
def test(sequence , n, patternlen=None, initblocks=None):
    ...
    for i in range(Q):             # Mark final position of
        pattern =sequence[i*L:(i+1)*L] # each pattern
        idx = int(pattern, 2)
    ...
    sigma = abs((fn - ev_table[L])/((sqrt(var_table[L]))*sqrt(2)))
    P = erf(sigma)
