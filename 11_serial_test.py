...
def test(sequence, n, patternlen=None):

    # Pattern length
    if patternlen != None:
        m = patternlen  
    else:  
        m = int(floor(log(n,2)))-2
    ...
    # Step 1
    padded_input=sequence[0:n]+sequence[0:m-1]
    ...
    p1 = gammainc(2**(m-2),delta1/2.0)
    p2 = gammainc(2**(m-3),delta2/2.0)
