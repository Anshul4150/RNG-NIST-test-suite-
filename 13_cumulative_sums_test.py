...
def normcdf(n):
    return 0.5 * erf(-n * sqrt(0.5))

def p_value(n,z):
    sum_a = 0.0
    startk = int(floor((((float(-n)/z)+1.0)/4.0)))
    endk   = int(floor((((float(n)/z)-1.0)/4.0)))
    for k in range(startk,endk+1):
        c = (((4.0*k)+1.0)*z)/sqrt(n)
    ...
    sum_b = 0.0
    startk = int(floor((((float(-n)/z)-3.0)/4.0)))
    endk   = int(floor((((float(n)/z)-1.0)/4.0)))
    for k in range(startk,endk+1):
        c = (((4.0*k)+3.0)*z)/sqrt(n)
        #d = scipy.stats.norm.cdf(c)
        d = normcdf(c)
        c = (((4.0*k)+1.0)*z)/sqrt(n)
        #e = scipy.stats.norm.cdf(c)
